...
class table_admin extends admin {
    ...
    function getfield_action() {
        ...
        $code='<table border="0" cellspacing="0" cellpadding="0" id="table_field" width="100%">';
        foreach ($field as $f) {
            $name=$f['name'];
            ...
            if (!isset($data[$name]))
                $data[$name]='';
            $code .= '<tr>';
            $code .= '<td class="left" width="19%">'.setting::$var['archive'][$name]['cname'].'</td><td width="1%">&nbsp;</td>';
            $code .= '<td width="70%">';
        ...
        if (front::post('batch') &&front::post('select')) {
            ...
            elseif (front::post('batch') == 'delete') {
                ...
                if ($delete >0){
                    ...
                    event::log("删除{$this->tname}",'成功 id='.implode(',',front::post('select')));
                }else
