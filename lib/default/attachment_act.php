...
class attachment_act extends act {
    ...
    function attachment_js_action() {
        ...
        if(!rank::arcget($aid,$this->view->usergroupid,'down')) {
            $link="<a id='att' href='javascript:alert(\"未取得授权,无法下载!\");'><img src='{$base_url}/images/download.gif' alt='$name' /></a>";
            echo tool::text_javascript($link);
            exit;
        }else {
            if(config::get('verifycode')) {
                $link="<a target='_blank' id='att' href='".url::create('attachment/downfile/aid/'.$aid.'/v/ce')."'><img src='{$base_url}/images/download.gif' alt='$name' /></a>";
                echo tool::text_javascript($link);
                exit;
            }else {
                $link="<a target='_blank' id='att' href='".url::create('attachment/down/aid/'.$aid)."'><img src='{$base_url}/images/download.gif' alt='$name' /></a>";
                echo tool::text_javascript($link);
                exit;
            }
        }
    ...
    function downfile_action() {
        ...
        if(front::post('submit')) {
            if(!session::get('verify') ||front::post('verify')<>session::get('verify')) {
                ...
                if(!rank::arcget($aid,$this->view->usergroupid,'down'))
                    $link="<br /><br /><br /><br /><br /><p align='center'><a id='att' href='javascript:alert(\"未取得授权,无法下载!\");'><img src='{$base_url}/images/download.jpg' alt='$name' border='0' /><br /><br />点击下载</a></p>";
                else $link="<br /><br /><br /><br /><br /><br /><p align='center'><a id='att' href='".url::create('attachment/down/aid/'.$aid)."'><img src='{$base_url}/images/download.jpg' alt='$name' border='0' /><br /><br />点击下载</a></p>";
                echo $link;
                exit;
            }
        }
