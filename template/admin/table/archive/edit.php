...
<td width="1%">&nbsp;</td>
<td>{form::getform('catid',$form,$field,$data)}
<span class="hotspot" onmouseover="tooltip.show('选择内容所在栏目，如果栏目有子级栏目，请选择子级栏目！');" onmouseout="tooltip.hide();"><img src="{$skin_path}/images/remind.gif" alt="" width="14" height="20"  style="margin-left:10px; margin-right:5px;"></span>
    ...
    <tr>
		...
		<span class="hotspot" onmouseover="tooltip.show('填写内容标题！');" onmouseout="tooltip.hide();"><img src="{$skin_path}/images/remind.gif" alt="" width="14" height="20"  style="margin-left:10px; margin-right:5px;"></span><input id="color" name="color" value="{$data['color']}"/> <input name="strong" type="checkbox" value="1" {if $data['strong']==1}checked{/if} />
		加粗
		</td>
    </tr>
    <tr>
...
<div id="divStatus"></div>
  ...
  <tr>
	 ...
	 <tr>
        ...
        <td>
		   <!--<select id='tag_option'>
		     <option value="0">请选择tag标签</option>
             {loop $tag_opton $t}
    ...
    <tr>
        ...
        <td width="1%">&nbsp;</td>
        <td><input type="text" name="adddate" id="adddate" value="{$data[adddate]}" class="skey" /><span class="hotspot" onmouseover="tooltip.show('内容发布时间，可自定义，格式：2010-08-08 08:08:08！');" onmouseout="tooltip.hide();"><img src="{$skin_path}/images/remind.gif" alt="" width="14" height="20"  style="margin-left:10px; margin-right:5px;"></span>
		</td>
    </tr>
		<tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="table2">
    ...
    <tr>
		...
		<span class="hotspot" onmouseover="tooltip.show('你可以自定生成文件名称<br />例如：abc.html<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>{</b>$caturl<b>}</b>/abc.html<br />默认：<b>{</b>$caturl<b>}</b>/show_<b>{</b>$aid<b>}</b>(_<b>{</b>$page<b>}</b>).html<br /><b>{</b>$caturl<b>}</b> = 栏目目录<br /><b>{</b>$aid<b>}</b> = 内容ID<br /><b>{</b>$page<b>}</b> = 内容分页值！');" onmouseout="tooltip.hide();"><img src="{$skin_path}/images/remind.gif" alt="" width="14" height="20"  style="margin-left:10px; margin-right:5px;"></span>
		</td>
    </tr>
    <tr>
...
<img src="{$skin_path}/images/remind.gif" alt="" width="14" height="20"  style="margin-left:10px; margin-right:5px;">
    ...
    <tr>
        ...
        <td>
            <div id=uploadarea>
            <div id="pvpics"><?php $ic=0;foreach($data as $k => $v){
				if(preg_match('/^pics([\d]+)$/',$k,$out) && $v){
					$ic++;
				?>
					<div id="pics{$ic}_up" style="clear:both;"><span id="pics{$ic}_preview"><img width="150" style="width:150px;" src="{$v}" border="0" /></span><div class="blank10"></div><input id="pics{$ic}" value="{$v}" class="input" name="pics{$ic}" style="float:left;" /> <input style="float:left;" id="pics{$ic}_del" onclick="pics_delete('{$ic}','pics');" value="" type="button" name="delbutton" class="a_del" /></div>
				<?php 
				}
...
<?php } ?>
    ...
    <tr>
        ...
        <td width="1%">&nbsp;</td>
        <td>
            <?php front::$get['province_id']=$data['province_id'];front::$get['city_id']=$data['city_id'];front::$get['section_id']=$data['section_id'];echo form::select('province_id',
    get('province_id') ? get('province_id') : 0, area::province_option()); ?>
<?php echo form::select('city_id', get('city_id') ? get('city_id') : 0,
        area::city_option()); ?>
